...
namespace WebAppCRUD.Admin
{
    public partial class ViewSuppliers : System.Web.UI.Page
    {
        ...
        {
            // This event is fired after the ObjectDataSource has returnd from performing an Insert.
            ;
        }
        ...
        {
            ; // After call to the BLL
            if(e.Exception != null)
            {
                Exception inner = e.Exception;
                while (inner.InnerException != null)
                    inner = inner.InnerException;
                string message = $"Problem inserting: {inner.GetType().Name}" +
                    $"<blockquote>{inner.Message}</blockquote>";
